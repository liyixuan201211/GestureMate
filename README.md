...
> ## 📌 接手请先读 [交接文档.md](交接文档.md)
>
> 项目现在位于 `~/南客松-v2/GestureMate`。交接文档里有：五分钟跑起来、
> 目录地图、两条推理路径的架构、**哪些是真验证过的**、坐标/镜像/手性等关键约定、
> 已知问题与下一步、以及与 `x5-link` / `3dgs-lab` 的关系。
>
> 在这个上游项目之上加了三层：
>
> | 新增 | 入口 | 说明 |
> |---|---|---|
> | **WebUI（浏览器取流 + WebRTC）** | `./run_webui.sh` → <http://127.0.0.1:8770> | 默认推理在浏览器里跑（MediaPipe Tasks / WebGL），视频不上行；详见 [WebUI-RTC.md](WebUI-RTC.md) |
> | **全景跟随与动作检测** | `.venv/bin/python -m pano --frames DIR --out OUT` | 在 X5 的等距柱状全景里找到人、跟住他、对他做手/脸/身检测；详见 [全景跟随与动作检测.md](全景跟随与动作检测.md) |
> | **动作识别：左移 / 右移 / 跳 / 蹲** | `.venv/bin/python -m action --demo`，或 `./run_webui.sh` 后看右侧「动作识别」卡片 | 摄像头前四个动作 → 事件流；迟滞、边沿触发、锁存、不应期、回位门控全在识别侧。**WebUI 里已接好**（真摄像头可用，并能现场调阈值）。详见 [动作识别-左移右移跳蹲.md](动作识别-左移右移跳蹲.md) |
>
> 本机（M2 Max / macOS）部署与 mps 优化见 [部署与优化.md](部署与优化.md)：
> 一键启动方式、修掉的两个「跑不起来」的 bug（`logging.info(..., end="")` 崩溃、
> match 计数每帧被清零），以及 `mediapipe.python.solutions` 各项优化的实测数据。
>
> ✅ 项目**现在有 git 仓库了**（本次做动作识别之前补的基线快照，原先确实没有）：
> `main` 上是那份基线，新功能在 `feat/action-lane-jump-squat` 分支上，改坏了可整体
> 回滚。`.venv/`（931MB，`run.sh` 会自动重建）与 `.DS_Store` 已 gitignore。
>
> 上游用法：`./run.sh`（首次会自动建 venv 并装依赖）
