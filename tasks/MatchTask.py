...
class MatchTask(Task):
    ...
    def _listen(self, x):
        logging.debug(f"poses {str(self.poseName)}:")
        for i in range(len(self.pose)):
            ...
            if delta != -1 and delta < self.sensetive[i]:
                logging.debug(msg + " match")
                self.count[i] += 1
                if self.count[i] >= self.frames[i]:
                    self.process(x)
                    return
            else:
                # 修 bug：原来这一行在 if 之外，等于**每帧都清零**，
                # 计数永远到不了 frames[i]。而计算器 demo 的 frames 全是 4，
                # 于是所有 match 任务实际上永远不会触发。语义应当是
                # 「连续 frames[i] 帧命中」，所以只在没命中时清零。
                self.count[i] = 0
                logging.debug(msg)
